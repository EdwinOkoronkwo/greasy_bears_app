...
export class SearchLocationComponent implements OnInit, OnDestroy {
  ...
  async choosePlace(place: any) {
    this.global.showLoader();
    console.log(place);
    if (this.from) {
      const savedPlace = await this.savedPlaces.find(
        (x) => x.lat === place.lat && x.lng === place.lng
      );
  ...
  async getCurrentPosition() {
    try {
      ...
      this.global.errorToast(
        'Check whether GPS is enabled & the App has its permissions',
        5000
      );
    }
  }
